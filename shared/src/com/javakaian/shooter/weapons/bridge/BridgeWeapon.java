...
public abstract class BridgeWeapon {
    ...
    public void fire() {
        ...
        // Do not mutate ammo on client; server is authoritative and will send updates.
        // Keep local gating minimal to avoid spamming when obviously empty (optional).
        if (currentAmmo <= 0) {
            System.out.println(name + " is out of ammo! Waiting for reload update.");
            return;
        }

        // Bridge Pattern: Delegate to the implementation
        firingMechanism.startFire(name, damage);
    }
    ...
    public void setAmmoCapacity(int ammoCapacity) {
        ...
        // Do not auto-refill on capacity changes; clamp to new max
        if (currentAmmo > this.ammoCapacity) {
            currentAmmo = this.ammoCapacity;
        }
    }
